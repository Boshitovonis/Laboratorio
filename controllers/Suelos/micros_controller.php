<?php
require_once __DIR__ . '/../../models/conexion.php';
require_once __DIR__ . '/../../models/Suelos/micros_model.php';

$resultado = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $peso    = (float) $_POST['peso'];
    $conc_cu = (float) $_POST['conc_cu'];
    $conc_zn = (float) $_POST['conc_zn'];
    $conc_fe = (float) $_POST['conc_fe'];
    $conc_mn = (float) $_POST['conc_mn'];
    $conc_k  = (float) $_POST['conc_k'];
    $blk_cu  = (float) $_POST['blk_cu'];
    $blk_zn  = (float) $_POST['blk_zn'];
    $blk_fe  = (float) $_POST['blk_fe'];
    $blk_mn  = (float) $_POST['blk_mn'];
    $blk_k   = (float) $_POST['blk_k'];
    $control = (float) $_POST['control'];


    //Cálculos a realizar
    $ppm_cu = (($conc_cu - $blk_cu)*25)/$peso ?? 0;
    $ppm_zn = (($conc_zn - $blk_zn)*25)/$peso ?? 0;
    $ppm_fe = (($conc_fe - $blk_fe)*25)/$peso ?? 0;
    $ppm_mn = (($conc_mn - $blk_mn)*25)/$peso ?? 0;
    $ppm_k  = (($conc_k  - $blk_k)*25)/$peso ?? 0;

    $resultado = guardarMicros(
        $peso,
        $conc_cu, $conc_zn, $conc_fe, $conc_mn, $conc_k,
        $blk_cu,  $blk_zn,  $blk_fe,  $blk_mn,  $blk_k,
        $ppm_cu,  $ppm_zn,  $ppm_fe,  $ppm_mn,  $ppm_k, $control
    );

    $resultado['ppm_cu'] = $ppm_cu;
    $resultado['ppm_zn'] = $ppm_zn;
    $resultado['ppm_fe'] = $ppm_fe;
    $resultado['ppm_mn'] = $ppm_mn;
    $resultado['ppm_k']  = $ppm_k;
}

require_once __DIR__ . '/../../view/Suelos/micros_view.php';
?>
